...
layui.use(['element', 'laydate', 'form', 'table', "layer", "extension", "urlrelated"], function () {
    ...
    $("#btn_reset").on("click", function () {
        ...
        // debugger
        // daystart.config.value = new Date(new Date() - 6 * 24 * 3600 * 1000);
    ...
    form.on('radio(timeType)', function (data) {
        ...
        if (data.value == "month") {
            ...
            // daystart.config.value = new Date();
            $("#ss_cjsjWeek,#ss_cjsjMonth,#ee_cjsjWeek,#ee_cjsjMonth").hide();
    ...
    function RenderZheEcharts() {
        ...
        var colinfozhe = [
            ...
            , { field: 'count', minWidth: 140, title: '核查量' }
            , { field: 'successCount', minWidth: 140, title: '核查量(比中)' }
            , { field: 'failCount', minWidth: 140, title: '核查量(未比中)' }
            , { field: 'totalCount', minWidth: 140, title: '核查总量' }
        ]
    ...
    function RenderZhuEcharts() {
        ...
        for (var i = 0; i < echartszhudata.data.length; i++) {
            ...
            seriesDataz.push(echartszhudata.data[i].successCount);
            seriesDataz2.push(echartszhudata.data[i].failCount);
        }
        var addzhuData = {
        ...
        var colinfozhu = [
            { field: 'jgmc', minWidth: 140, title: '机构' }
            , { field: 'count', minWidth: 140, title: '核查量' }
            , { field: 'successCount', minWidth: 140, title: '核查量(比中)' }
            , { field: 'failCount', minWidth: 140, title: '核查量(未比中)' }
            , { field: '', minWidth: 140, title: '操作', align: 'center', fixed: 'right', toolbar: '#chartsZhu-table-operate' }
        ]
    ...
    table.on('tool(chartsZhu-table-operate)', function (obj) {
        ...
        if (obj.event === 'detail') {
            ...
            layer.open({
                ...
                area: ['800px', '600px'],
                resize: false,
                content: ['./identify_sbl_detail.html?name' + data.jgmc, "no"],
                btn: ['取消'],
                yes: function (index, layero) {
                    var iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
                    // var selectTreeOrg = iframeWin.getSelectOrg();
